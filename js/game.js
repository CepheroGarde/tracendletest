...
function startArchiveGame(dateString) {
  ...
  sessionState.maxGuesses = 5;

  sessionState.target = getTargetForDate(archiveDate, dataList);
...
function startGame(mode) {
  ...
  if (mode === 'daily') {
    ...
    if (saved && saved.targetName && saved.guesses?.length > 0 && !saved.isGameOver) {
  ...
  // ---- Hard-mode: generate clues if needed ----
  if (mode === 'hard' && sessionState.clues.length === 0) {
    const pData      = allPersistentData[currentGameType];
  ...
  const modesWithHistory = ['daily', 'ranked', 'unlimited', 'hard'];
